    ...
    def from_value(self, value: Any) -> Any:
        if not value:
            if not self.nullable:
                raise ValueError(f"Not nullable: неправильно указано значение для поля {self.__class__.__name__}")
        if hasattr(self, "__template__") and self.__template__:
            if not re.match(f"^{self.__template__}$", str(value)):
                ...
                raise ValueError(f"Invalid value {self.__class__.__name__}: {self.__template__}{example}")
        if hasattr(self, "custom_validator") and isinstance(self.custom_validator, Callable):
...
class DateField(Field):
    ...
    def custom_validator(self, value) -> None:
        try:
            ...
            raise ValueError(f"Invalid {self.__class__.__name__} {value} (example: {self.__example__}")
...
class BirthDayField(DateField):
    ...
    def custom_validator(self, value) -> None:
        ...
        if datetime.datetime.now() - value_datetime > valid_age:
            raise ValueError(f"Invalid age {value} (Age must be less than 70 years)")
